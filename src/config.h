 ...
 */
#define MACRO_MAX_SIZE 40 // max number of chars allowed for a macro
...
/**
 * @brief change sdcard behavior
 *
 * @param SD_ENABLED 1(enable) 0(disable) micro_sd (macros are loaded from default_profiles.h)
 * @param DEFAULT_FILE set the default file to load macros from
 * @param LOAD_BEHAVIOR set the behavior of the macros when they are not found in the sdcard
 *         * LOAD_BEHAVIOR 1(default) macros are loaded from sdcard if it present else from default_profiles.h
 *         * LOAD_BEHAVIOR 0 macros are always loaded from sdcard
 */

#define SD_ENABLED 1
#define DEFAULT_FILE "macros.txt"
#define LOAD_BEHAVIOR 1
